...
def _query_opencode_sessions(opencode_db: str) -> list:
    ...
    for s in sessions:
        ...
        s["created_at"] = (
            _dt.datetime.fromtimestamp(ts / 1000, tz=_dt.UTC).isoformat() if ts else None
        )
    return sessions
...
def cmd_init(args):
    ...
    rational.add_fact(
        "session_id", session_id, agent_id="hermes", predicate="active", source="hermes_cli"
    )
...
def cmd_save(args):
    ...
    if args.type == "fact":
        ...
        subject = getattr(args, "subject", None) or (
            data.get("subject", "unnamed") if data else "unnamed"
        )
        obj = getattr(args, "object", None) or (data.get("object", "") if data else "")
        predicate = getattr(args, "predicate", None) or (
            data.get("predicate", "is") if data else "is"
        )
        store.add_fact(
            subject, obj, agent_id="hermes", predicate=predicate, source=f"session:{session_id}"
        )
        ...
        store.log(
            ...
            agent_id="hermes",
            detail=detail,
            outcome=data.get("outcome", "pending") if data else "pending",
        )
        op = OperationalStore()
    ...
    try:
        ...
        if getattr(args, "deep", False):
            ...
            output["deep_audit"] = {
                "axis": 13,
                "patterns": patterns,
                "recommendation": "High correlation with previous build patterns detected."
                if patterns.get("sessions", 0) > 10
                else "Insufficient cross-session data.",
            }
    except Exception as e:
...
def cmd_close(args):
    ...
    rational = MnemosyneRationalStore()
    rational.add_fact(
        "session_closed", session_id, agent_id="hermes", predicate="was", source="hermes_cli"
    )
...
def main():
    ...
    load_p = subparsers.add_parser(
        "load", help="Load context from Mnemosyne + optional OpenCode DB"
    )
    load_p.add_argument("--session", required=True, help="Session ID to load")
    load_p.add_argument(
        "--opencode-db", default=None, help="Path to OpenCode SQLite DB (D:\\opencode\\opencode.db)"
    )
    ...
    save_p.add_argument(
        "--type", required=True, choices=["fact", "event", "goal"], help="Data type"
    )
    save_p.add_argument(
        "--data", default=None, help="JSON data string (alt: use --subject/--object)"
    )
    save_p.add_argument(
        "--subject", default=None, help="Fact subject (use with --type fact, avoids JSON)"
    )
    ...
    search_p.add_argument(
        "--deep", action="store_true", help="Include Axis 13 deep pattern analysis"
    )
    ...
    list_p = subparsers.add_parser(
        "list", help="List recent Hermes sessions + optional OpenCode DB"
    )
    list_p.add_argument(
        "--opencode-db", default=None, help="Path to OpenCode SQLite DB (D:\\opencode\\opencode.db)"
    )
