...
def test_meta_records():
    ...
    print("--- Meta-Cognition (Axis 8) Test ---")
    ...
    store.record(agent_id=test_agent, score=0.95, flaws=["none"], session_id=test_session)

    # 2. Test adjust_reliability()
    print(f"Adjusting reliability for {test_agent}...")
    store.adjust_reliability(agent_id=test_agent, delta=-0.05, violation_type="test_violation")
    ...
    cursor.execute(
        "SELECT draft_quality, session_id FROM meta_cognition WHERE agent_id = ? ORDER BY id DESC LIMIT 1",
        (test_agent,),
    )
    ...
    if res_record and res_record[0] == 0.95:
        ...
        print("  [FAIL] Meta record missing or incorrect.")
    ...
    cursor_rel.execute(
        "SELECT reliability_score FROM agent_reliability WHERE agent_id = ?", (test_agent,)
    )
    ...
    if res_rel:
        ...
        print("  [FAIL] Reliability score missing in reliability.db.")
