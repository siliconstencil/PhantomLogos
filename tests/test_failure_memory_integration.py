...
async def test_gnosis_injection():
    ...
    assert any("MANDATORY RULE" in r['prevention_rule'] for r in rules_in_store), "Rule failed to commit to ReflectionStore"
    ...
    print(f"\nDEBUG CONTEXT START\n{context_str}\nDEBUG CONTEXT END\n")
    assert "### MNEMOSYNE AXIS 8 (PREVENTION RULES)" in context_str, f"Header missing in context. Context sample: {context_str[:200]}"
...
async def test_evaluator_trigger():
    ...
    try:
        res = await asyncio.wait_for(evaluator.evaluate(poor_draft, contract={"threshold": 0.7}), timeout=15.0)
