...
async def test_pruning():
    ...
    with store._get_conn() as conn:
        ...
        c.execute("INSERT INTO failure_memory (error_type, root_cause, prevention_rule, severity, recurrence_count, status, context_hash, updated_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                  ("test_old", "old error", "Old prevention rule", 1, 5, "active", "deadbeef00000001", old_ts, old_ts))
        c.execute("INSERT INTO failure_memory (error_type, root_cause, prevention_rule, severity, recurrence_count, status, context_hash, updated_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                  ("test_recent", "recent error", "Recent prevention rule", 3, 2, "active", "deadbeef00000002", recent_ts, recent_ts))
        conn.commit()
