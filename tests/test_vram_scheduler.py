...
async def test_model_fitting():
    ...
    assert find_fitting_model("critique", 2.8) == "phi-4-mini-ud:latest"
...
async def test_context_pruner_tiers():
    ...
    mems = [
        {"text": f"Memory {i}", "importance": 0.5 + i * 0.05, "timestamp": time.time()}
        for i in range(10)
    ]
    result = await pruner.prune_by_tier(mems, "reasoning")
    assert len(result) > 0
...
async def test_token_pruner_overflow():
    ...
    result = await pruner.prune_context(huge_mem, token_limit=100)
