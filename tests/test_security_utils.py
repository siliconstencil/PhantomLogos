...
from src.utils.security_utils import load_secrets_to_env, validate_cloud_key, set_secret
...
def test_load_secrets_keyring_success(mock_get):
    mock_get.side_effect = lambda service, key: "AIza-from-keyring-12345678901234567890123456789" if key == "GEMINI_API_KEY" else None
    load_secrets_to_env()
...
def test_load_secrets_fallback(mock_getenv, mock_get_keyring):
    ...
    mock_getenv.side_effect = lambda key, default=None: "AIza-from-env-12345678901234567890123456789" if key == "GEMINI_API_KEY" else None
    load_secrets_to_env()
