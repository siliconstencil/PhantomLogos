...
from cognition.sophia.hephaestus import ReasoningState, SophiaOutput, TechnicalClaim
...
class TestSovereignTruthGuard(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_shadow_vram_lie(self, mock_vram):
        ...
        with patch("src.lachesis.verifiers.output_guard.OutputGuard.record_shadow_violation") as mock_record:
    ...
    async def test_hard_gate_blocking(self, mock_context):
        ...
        mock_context.return_value = ("Context text", {"block": True, "reason": "Severe Hallucination Loop Detected"})
    ...
    async def test_auto_rotation_trigger(self, mock_vram, mock_guard):
        ...
        # First failure
        store.adjust_reliability("sophia", -0.3, "shadow_verification_failed", session_id=self.session_id)
        self.assertNotIn("gemini-2.0-flash-thinking-exp:latest", BLACKLISTED_MODELS.get(self.session_id, []))
    ...
    async def test_schema_enforcement_fail(self):
        """S2: Test if Sophia rejects non-JSON / Invalid Schema output."""
        # This is harder to test without mocking the LLM response inside run_draft
        # But we can test the parsing logic in a smaller unit
        from cognition.sophia.sophia import strip_thinking_block
        
        # Invalid JSON
        bad_output = "I am a helpful assistant and I don't want to use JSON."
        
        with patch("src.architrave.gemini_client.AgnosticArchitrave.generate_async") as mock_gen:
