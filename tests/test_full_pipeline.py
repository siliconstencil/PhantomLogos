...
from cognition.sophia import (
    get_temperature, TaskRouter, ToolValidator, 
    get_dynamic_context, extract_tool_calls, strip_thinking_block
)
from cognition.mnemosyne import MemoryArbitrator
from src.architrave import resolve_model, resolve_local_model
from src.atropos import ContextPruner, AtroposMonitor, MatryoshkaEmbedding
...
async def test_tool_validator_json():
    tv = ToolValidator()
    data = tv.validate_json('{"is_valid": true, "flaws": ["ok"], "suggestions": []}',
...
async def test_tool_extraction_robustness():
    ...
    # Malformed JSON
    text_malformed = 'Bad {"tool": "ls", "input": "missing quote} Good {"tool": "vram", "input": ""}'
    calls_malformed = extract_tool_calls(text_malformed)
    assert len(calls_malformed) == 1 and calls_malformed[0]["tool"] == "vram"
    
    # Thinking block
    text_think = '<think>{"tool": "ls", "input": "."}</think>Final answer: {"tool": "report", "input": ""}'
