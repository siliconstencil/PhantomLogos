...
class SympyVerifier:
    ...
    def verify_math_expression(self, problem: str) -> str:
        ...
        math_keywords = ["solve", "calculate", "find", "determine", "what", "how", "evaluate", "hesapla", "nedir"]
        has_keywords = any(kw in problem.lower() for kw in math_keywords)
        if not has_keywords and re.search(r"^[0-9xyzXYZ\s\+\-\*\/\=\(\)\.]+$", problem) and len(problem) < 50:
            return "simple"
        if len(problem) < 400 and not any(kw in problem.lower() for kw in ["explain", "describe", "proof", "step-by-step", "adim adim"]):
            return "medium"
        return "complex"

    def verify_math_llm(self, problem: str, light: bool = False) -> Dict[str, Any]:
        return self.llm_engine.verify_math_llm(problem, light)
