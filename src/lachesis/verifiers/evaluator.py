...
class AdversarialEvaluator:
    ...
    def _originality_score(self, draft: str) -> float:
        boilerplate_hits = sum(
            len(re.findall(p, draft, re.MULTILINE)) for p in BOILERPLATE_PATTERNS
        )
    ...
    async def evaluate(
        ...
        if not draft or len(draft.strip()) < 5:
            result = {
                ...
                "flaws": ["Draft is empty or too short"],
        ...
        flaws = []
        if verification_score < 0.5:
            flaws.append(f"Invalid math (Axis 11 score {verification_score} < 0.5)")
        if design_score < 0.5:
            flaws.append(f"Design weak ({design_score})")
        if functionality_score < 0.5:
            flaws.append(f"Functionality insufficient ({functionality_score})")
        if citation_score < 0.5:
            flaws.append("Citation missing or poor ([SRC:axis_N] refs required)")
        if consistency_score < 0.5:
            flaws.append(f"Consistency/Hallucination risk detected ({consistency_score})")

        suggestions = []
        if citation_score < 0.8:
            suggestions.append("Add [SRC:axis_N] citations for every claim")
        if not re.search(r"def\s+\w+", draft):
            suggestions.append("Define at least one function")
    ...
    def _consistency_score(
        ...
        if tool_results:
            for res in tool_results:
                evidence_text += str(res.get("output", ""))[:500] + " "
        if anchors:
            evidence_text += anchors[:1000]
        ...
        match_ratio = (
            len(draft_words.intersection(evidence_words)) / len(draft_words) if draft_words else 1.0
        )
        if match_ratio >= 0.2:
            return 1.0
        if match_ratio >= 0.1:
    ...
    def _ocr_accuracy_score(self, draft: str, tool_results: list) -> float:
        ocr_outputs = [
            str(res.get("output", ""))
            for res in tool_results
            if "OCR Mode" in str(res.get("prompt", ""))
        ]
        if not ocr_outputs:
            return 1.0
