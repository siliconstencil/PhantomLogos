...
class OpenCodeStore:
    ...
    def get_session_messages(self, session_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return [{"id": r["id"], "time_created": r["time_created"], "data": r["data"]} for r in rows]
        except Exception as e:
            logger.warning(f"OpenCodeStore (Axis {self.AXIS_ID}): get_session_messages failed ({e})")
            return []
        finally:
            conn.close()
    ...
    def get_cross_session_patterns(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"OpenCodeStore (Axis {self.AXIS_ID}): get_cross_session_patterns failed ({e})")
            return {"sessions": 0, "top_agents": [], "top_models": [], "avg_messages_per_session": 0}
        finally:
            conn.close()
