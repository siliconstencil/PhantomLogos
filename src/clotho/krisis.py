...
def get_hermes_bridge_context(session_id: str) -> str:
    ...
    try:
        ...
        for task_type in ("critique", "vision", "semantic"):
            ...
            if best:
                ctx += f"Best tool for [{task_type}]: {best[0][0]} (success rate: {best[0][1]:.0%})\n"
...
def should_use_tier(state: Any):
    ...
    if tier == 3:
        ...
        if expert_model in current_blacklisted:
            logger.warning(f"krisis: Primary Expert model '{expert_model}' is BLACKLISTED. Rotating to fallback.")
...
def should_call_tools(state: Any):
    ...
    if not draft:
        if verify_retry < 3:
            logger.warning(f"krisis: Draft cleared by verification (Retry {verify_retry}). Routing to loopback.")
            return "expert_draft" if state.get("ru_flow_tier") == 3 else "draft"
        else:
            logger.warning(f"krisis: Verification max retry ({verify_retry}) reached with empty draft. Forcing critique.")
            return "critique"
...
def should_continue(state: Any):
    ...
    if zone == "red":
        logger.error(f"krisis: RED ZONE detected (Score: {critique.get('overall_score')}). Routing to deadlock_resolver.")
        return "deadlock_resolver"

    if is_valid:
        logger.info(f"krisis: Critique PASS (Zone: {zone}, iter={iteration}). Routing to reflection before refine.")
        return "reflection"

    if iteration >= max_iter:
        logger.warning(f"krisis: Max iterations reached ({iteration}/{max_iter}). Routing to reflection.")
        return "reflection"
    ...
    try:
        ...
        if reliability < 0.2:
            logger.error(f"FATAL VETO: Agent reliability critically low ({reliability:.2f}). Emergency Lockdown (END).")
            return END
        elif reliability <= 0.3:
            logger.warning(f"VETO: Agent reliability too low ({reliability:.2f}). Stopping loop via reflection.")
            return "reflection"
    except Exception as e:
        logger.warning(f"krisis: Meta-cognition reliability check bypassed ({e})")

    logger.info(f"krisis: Critique FAIL (iter={iteration}). Looping back to draft for adversarial refinement.")
    
    verify_retry = state.get("verification_retry", 0)
    if verify_retry >= 3:
        logger.error(f"krisis: Verification Max Retry reached ({verify_retry}). DEADLOCK DETECTED (Hallucination Spiral). Routing to deadlock_resolver.")
        return "deadlock_resolver"
...
def should_after_reflection(state: Any):
    ...
    if is_valid:
        # Phase 11.21.3: Adaptive Skip - Skip Refine if confidence is exceptionally high (> 0.9)
        if confidence >= 0.9:
            logger.info(f"krisis: ADAPTIVE SKIP - High confidence ({confidence:.2f}) detected. Bypassing refine phase.")
            return "end"
        return "refine"
