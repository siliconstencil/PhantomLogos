...
async def anchor_inject_node(state: Any):
    ...
    try:
        ...
        # Pillar 1: Injection-based context (Düzeltme 1)
        ...
        if agent_id:
            try:
                ...
                try:
                    ...
                    reranked = await asyncio.to_thread(reranker.rerank, state["task"], skill_texts, top_n=3)
                    top_skills = []
                ...
                for sk_name in top_skills:
                    ...
                    if ctx:
                        builder.add_fragment(fragment_id=f"skill_{sk_name}", content=ctx, axis=8, precedence=130)
            except Exception as e:
        ...
        return {"anchors": anchors_xml, "memory_sync": memory_sync, "spatial_context": spatial_ctx}
