...
class ModelLoader:
    ...
    def __init__(self, base_url: Optional[str] = None):
        self.base_url = base_url or os.getenv("OLLAMA_BASE_URL", "http://localhost:11434").replace("/v1", "")
        ...
        try:
            ...
            if not ps_models:
                if self._loaded_model is not None:
                    logger.info(f"ModelLoader: ollama ps empty, clearing stale _loaded_model")
            ...
            for name in loaded_names:
                if name in CORE_MODELS:
                    if self._loaded_model != name:
                        logger.info(f"ModelLoader: Synced _loaded_model -> '{name}' (from ollama ps)")
    ...
    def load(self, model_name: str, timeout_s: int = 30) -> bool:
        ...
        try:
            ...
            if "cuda" in err_str and ("out of memory" in err_str or "oom" in err_str):
                logger.critical(f"Morpheus: CUDA OOM detected loading {model_name}. Forcing emergency flush...")
                self.flush()
                time.sleep(2)
