...
from .hephaestus import (
    _get_monitor, _get_procedural, _get_meta, _get_episodic,
    ReasoningState, CritiqueResult, SophiaOutput, TechnicalClaim,
    strip_thinking_block, extract_tool_calls,
    extract_first_json_block, get_sophia_instructions
)
...
async def run_draft(state: ReasoningState, thinking: bool = True) -> Union[str, tuple]:
    global _consecutive_draft_timeouts
    logger.info(f"Sophia: Generating Draft (Thinking Mode: {thinking})...")
    stable, dynamic, block_signal = await get_dynamic_context("sophia", task_hint=state.task, session_id=state.session_id)
    ...
    if block_signal["block"]:
        ...
        if hasattr(state, 'flags') and state.flags.get('bypass_memory_gate'):
            ...
            return f"HARD GATE: Task blocked due to previous failures. Reason: {block_signal['reason']}", []

    # Load tool whitelist and instructions
    try:
        with open(os.path.join("agent", "sophia.yaml"), "r") as f:
    ...
    try:
        ...
        if output.startswith("[SYSTEM GUARD]"):
            logger.warning(f"Sophia: System Guard message detected. Bypassing JSON parse.")
            return output, []
        ...
        try:
            ...
            logger.info(f"Sophia: Parsed structured output with {len(tool_calls)} tool calls and {len(parsed.technical_claims)} claims")
        ...
        check = guard.check(output, agent_id="sophia", context={
        ...
        if parsed and parsed.final_response:
            _get_episodic().log(
                session_id=state.session_id,
                agent_id="sophia",
                action="run_draft",
                detail=f"Task: {state.task[:100]}...",
                outcome="success"
            )
        ...
        logger.error(f"Sophia: Draft generation timed out after 60s ({_consecutive_draft_timeouts}x)")
        if _consecutive_draft_timeouts >= 3:
            logger.error("Sophia: 3 consecutive timeouts. GPU likely hung. Raising.")
            raise RuntimeError(f"Sophia: 3 consecutive draft timeouts. GPU hang suspected.")
        return "", []
...
@_get_monitor().trace("sophia")
async def run_critique(draft: str, session_id: str = "default") -> CritiqueResult:
    ...
    model_name = scheduler.request_model("critique") or "phi-4-mini-reasoning-ud-q5_k_xl:latest"
    
    response = await asyncio.wait_for(
        get_ollama_client().chat(
            model=model_name,
            messages=[
                {"role": "system", "content": f"You are the Logical Auditor.\n\nCONTEXT:\n{context}\n\nReturn JSON: is_valid, flaws, suggestions."},
    ...
    try:
        ...
        if not json_str:
            return CritiqueResult(is_valid=False, flaws=["Parse Error"], suggestions=[content], confidence_score=0.1)
        ...
        return CritiqueResult(is_valid=False, flaws=[str(e)], suggestions=[content], confidence_score=0.1)

@_get_monitor().trace("sophia")
async def run_refine(task: str, draft: str, critique: CritiqueResult, session_id: str = "default") -> str:
    ...
    critique_str = json.dumps(critique) if isinstance(critique, dict) else (critique.model_dump_json() if hasattr(critique, 'model_dump_json') else str(critique))
    ...
    guard = get_output_guard()
    check = guard.check(result_text, agent_id="sophia", context={
        "function": "run_refine",
        "require_timestamp": True,
        "session_id": session_id
    })
    if check["violations"]:
        _get_meta().adjust_reliability("sophia", check["score_delta"], check["violations"][0], session_id=session_id)
        if check["action"] == "reject":
