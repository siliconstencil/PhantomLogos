...
class SessionLog:
    ...
    """
    def __init__(self, session_id: str, store: Optional[EpisodicStore] = None):
    ...
    def compact(self, max_tokens: int = 8000) -> Dict[str, Any]:
        ...
        summary_parts = []
        if task_events:
            tasks = [e["payload"].get("objective", "")[:120] for e in task_events if isinstance(e.get("payload"), dict)]
        ...
        if tool_events:
            ...
            summary_parts.append(f"Tools used ({len(tool_events)} calls): {', '.join(sorted(tool_names))}")
        if audit_events:
            scores = []
        ...
        summary = " | ".join(summary_parts) if summary_parts else "No significant events in middle range."
        return {
            "summary": summary,
            "event_count": len(history),
