...
class TrajectorySession(MnemosyneBase):
    ...
    task: str = Column(Text)
    total_steps: int = Column(Integer, default=0)
    cumulative_reward: float = Column(Float, default=0.0)
    final_score: float | None = Column(Float)
    total_tokens: int = Column(Integer, default=0)
    total_latency_ms: float = Column(Float, default=0.0)
    created_at: datetime.datetime = Column(DateTime, default=lambda: datetime.datetime.now(datetime.UTC))
    completed_at: datetime.datetime | None = Column(DateTime)
...
class TrajectoryStep(MnemosyneBase):
    ...
    id: int = Column(Integer, primary_key=True)
    session_id: str = Column(String(64), nullable=False, index=True)
    trajectory_id: int = Column(Integer, ForeignKey("trajectory_sessions.id"))
    step_index: int = Column(Integer, nullable=False)
    node_name: str = Column(String(100), nullable=False)
    reward: float = Column(Float, default=0.0)
    score: float | None = Column(Float)
    tokens_used: int = Column(Integer, default=0)
    latency_ms: float = Column(Float, default=0.0)
    tier: int | None = Column(Integer)
    model_tier: str | None = Column(String(50))
    created_at: datetime.datetime = Column(DateTime, default=lambda: datetime.datetime.now(datetime.UTC))
