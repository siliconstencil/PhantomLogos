...
class TemporalStore:
    ...
    def __init__(self, db_path: str | None = None):
        ...
        self._db_path = db_path or to_absolute_path("data/mnemosyne.db")
    ...
    def _ensure_table(self) -> None:
        ...
        try:
            ...
            if "valid_from" not in columns:
                ...
                conn.execute(
                    "UPDATE temporal_metrics SET valid_from = timestamp WHERE valid_from IS NULL"
                )
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_temp_validity ON temporal_metrics(valid_from, valid_until)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_temp_key ON temporal_metrics(event_key, valid_from DESC)"
            )
            conn.commit()
            logger.info(f"TemporalStore (Axis {self.AXIS_ID}): SQLite tables ready.")
        except Exception as e:
            logger.error(
                f"TemporalStore (Axis {self.AXIS_ID}): _ensure_table failed ({e})", exc_info=True
            )
        finally:
            conn.close()
    ...
    def record_with_supersede(
        self, session_id: str, event_type: str, event_key: str, **kwargs
    ) -> int:
    ...
    def query(
        ...
        try:
            ...
            logger.error(
                f"TemporalStore (Axis {self.AXIS_ID}): query failed for session {session_id} ({e})"
            )
            return []
        finally:
            conn.close()
    ...
    def get_fact_at(self, event_key: str, timestamp: float) -> dict[str, Any] | None:
